...
export function isSetFinished(player1Score: number, player2Score: number): boolean {
  return Math.max(player1Score, player2Score) >= 11 && Math.abs(player1Score - player2Score) >= 2;
}

export function getSetWinner(player1Score: number, player2Score: number): PlayerId | null {
